...
/** Legge un asset da public/ e lo restituisce come data URI. */
async function dataUri(publicPath) {
  const file = join(ROOT, 'public', publicPath.replace(/^(?:\.{0,2}\/)+/, ''));
...
const js = await readFile(join(TMP_DIR, 'assets/app.js'), 'utf8');
const css = await inlineAssets(await readFile(join(TMP_DIR, 'assets/app.css'), 'utf8'));
